...
function PriceRow({ title, subtitle, oldPrice, price, bullets, img, reversed = false }) {
  return (
      ...
      <div className={`grid md:grid-cols-2 gap-10 items-start ${reversed ? 'md:grid-flow-dense' : ''}`}>
        <div className={`${reversed ? 'md:order-2' : ''}`}>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <a className="inline-flex items-center px-5 py-2.5 rounded-xl bg-gradient-to-r from-[var(--mango-green)] to-[var(--india-green)] hover:from-[var(--yellow-green)] hover:to-[var(--india-green)] text-[var(--rich-black)] font-semibold shadow-lg transition" href="#">
              Create my Card
            </a>
            <a className="inline-flex items-center px-5 py-2.5 rounded-xl border hover:border-[var(--mango-green)] text-[var(--text-primary)] transition" style={{ borderColor: 'var(--dark-jungle-green)' }} href="#">
              Call Now
            </a>
          </div>
        </div>
  ...
  return (
    <div>
      <section className="mx-auto max-w-7xl px-6 pt-12">
        <h2 className="text-center text-3xl md:text-4xl font-bold text-[var(--text-primary)]">Product Pricing</h2>
      </section>
